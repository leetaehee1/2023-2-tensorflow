...
게임 유통 서비스인 Steam의 각종 게임에 달린 한국어 리뷰를 수집한 것이다. 게임 커뮤니티 특성 상 비속어 및 은어가 많이 사용된 것이 특징이다. 데이터는 탭으로 분리되어 있으며, 첫번째 필드에는 라벨 긍/부정(1=긍정, 0=부정), 두번째 필드에는 리뷰 텍스트가 위치한다. 긍정과 부정의 비율이 1:1에 가깝도록 샘플링된 자료이다.
...
|출력| 입력  |모델|
|-------------|---|---|
|부정(0), 긍정(1) | 스팀 게임 리뷰 문장|[KOELECTRA small](https://github.com/monologg/KoELECTRA) [3]|
...
|...|...|
|0|     노잼이네요... 30분하고 지웠어요...|
|1| 야생을 사랑하는 사람들을 위한 짧지만 여운이 남는 이야기. 영어는 그리 어렵지 않습니다.|
 |  1|          한국의 메탈레이지를 떠오르게한다 진짜 손맛으로 하는게임|
...
- **label** 분류 및 시각화
  ...
  ![label](https://github.com/leetaehee1/Koelectra_SteamReview/assets/79897716/caaff79e-33dc-472d-884f-f3eca2495a49)

  라벨을 분류하고 시각화를 해봤을때 0(부정) 49,957, 1(긍정) 49,936개로 1:1 비율로 나뉘어져 있다.

 - 전체 데이터의 문장길이 분포 확인
   
   ![문장길이의분포](https://github.com/leetaehee1/Koelectra_SteamReview/assets/79897716/7fecdf0c-71d4-4122-aec7-e7bf8518021d)

문장의 길이를 기준으로 문장을 분류한 결과, 다음과 같은 분포를 보였다.
  - 문장 길이 1 ~ 6: 35268개
  - 문장 길이 6 ~ 11: 24965개
  - 문장 길이 11 ~ 16: 14711개
  - 문장 길이 16 ~ 21: 10101개
...
주요 특징으로는 문장의 길이는 1 ~ 6 단어로 이루어진 문장이 가장 많고, 그 다음으로 6 ~ 11 단어, 11 ~ 16 단어의 순으로 분포한다. 문장의 길이가 31 단어 이상인 문장은 전체의 4.2%에 불과하다.    
분포 결과를 보면, 전체 문장의 67.5%가 1 ~ 6 단어로 이루어진 문장으로, 이는 짧은 문장이 텍스트에서 일반적으로 많이 사용됨을 의미한다. 또한, 문장의 길이가 31 단어 이상인 문장은 전체의 4.2%에 불과한 것으로 보아, 긴 문장은 상대적으로 적게 사용되는 것으로 보인다.
